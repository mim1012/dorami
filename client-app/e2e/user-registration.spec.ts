...
test.describe('User Registration (Profile)', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    // /login은 public route이므로 redirect 없이 안정적으로 로드됨
  ...
  test('should show custom validation errors on submit', async ({ page }) => {
    ...
    // 필수 필드 에러 메시지 확인 (인스타그램 ID, 전화번호는 선택 필드로 변경됨)
  ...
  test('should validate format of ZIP code and phone number', async ({ page }) => {
    ...
    await page.getByLabel('전화번호 (미국, 선택)').fill('123');
  ...
  test('should complete registration with valid data', async ({ page }) => {
    ...
    await page.getByLabel('전화번호 (미국, 선택)').fill('2125551234');

    // 제출
    await page.getByRole('button', { name: '프로필 등록 완료' }).click();

    // 성공 시 홈 또는 라이브 페이지로 리다이렉트 (프로필 등록 기본 redirect: /live)
    await expect(page).toHaveURL(/\/(live)?$/, { timeout: 15000 });

    // ── API-UI verification: persisted profile data ────────────────────────
    try {
      ...
      if (profile) {
        const apiDepositorName: string = profile.depositorName ?? '';
        const apiFullName: string = profile.shippingAddress?.fullName ?? '';

        if (apiDepositorName === '테스트사용자') {
          console.log('[API-UI]', '✅ depositorName persisted correctly');
        } else {
          expect(apiDepositorName).toBe('테스트사용자');
        }

        if (apiFullName && apiFullName !== 'Test User') {
          expect(apiFullName).toBe('Test User');
        } else if (apiFullName === 'Test User') {
          console.log('[API-UI]', '✅ shippingAddress.fullName persisted correctly');
        }
      }
    } catch (err) {
      console.warn('[API-UI] profile persistence check failed:', err);
    }
  });

  test('should redirect incomplete users back to profile register when visiting protected pages', async ({
    page,
  }) => {
    await page.goto('/cart', { waitUntil: 'domcontentloaded' });
    await expect(page).toHaveURL(/\/profile\/register/, { timeout: 10000 });
