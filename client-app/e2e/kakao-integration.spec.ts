...
test.describe('카카오 연동 테스트', () => {
  ...
  test.describe('1. 카카오 OAuth 로그인', () => {
    ...
    test('카카오 인증 API 엔드포인트 확인', async ({ page }) => {
      ...
      const response = await page.request.get('http://localhost:3001/api/v1/auth/kakao', {
        maxRedirects: 0,
      }).catch(() => null);
    ...
    test('카카오 콜백 엔드포인트 존재 확인', async ({ page }) => {
      ...
      const response = await page.request.get('http://localhost:3001/api/v1/auth/kakao/callback');
  ...
  test.describe('2. 카카오 로그인 후 사용자 정보', () => {
    test('Dev 로그인으로 카카오 사용자 시뮬레이션', async ({ page }) => {
      ...
      const response = await page.request.post('http://localhost:3001/api/v1/auth/dev-login', {
        data: {
    ...
    test('로그인 후 /auth/me 엔드포인트로 사용자 정보 확인', async ({ page }) => {
      ...
      const loginResponse = await page.request.post('http://localhost:3001/api/v1/auth/dev-login', {
        data: {
      ...
      if (meResponse.ok()) {
        ...
        console.log('⚠️ /auth/me는 쿠키 인증을 사용하지 않을 수 있음 (status:', meResponse.status(), ')');
  ...
  test.describe('3. 카카오톡 알림 전송', () => {
    ...
    test('주문 생성 시 카카오톡 알림 전송 로직 확인', async ({ page }) => {
      ...
      const orderResponse = await page.request.post('http://localhost:3001/api/v1/orders', {
        data: {
  ...
  test.describe('5. 카카오 프로필 정보 연동', () => {
    ...
    test('프로필 등록 페이지 확인', async ({ page }) => {
      await page.goto('http://localhost:3000/profile/register');
      ...
      if (await form.isVisible({ timeout: 3000 }).catch(() => false)) {
        ...
        // Instagram ID 입력 필드
        const instagramInput = page.getByLabel(/instagram|인스타/i).or(
          page.getByPlaceholder(/instagram|인스타/i)
        );
        ...
        // 예금주명 입력 필드
        const depositorInput = page.getByLabel(/예금주|입금자/i).or(
          page.getByPlaceholder(/예금주|입금자/i)
        );
  ...
  test.describe('6. 카카오 연동 환경 변수 확인', () => {
    test('필수 환경 변수 존재 확인', async ({ page }) => {
      ...
      console.log('   - KAKAO_CALLBACK_URL: 콜백 URL (예: http://localhost:3001/api/v1/auth/kakao/callback)');
