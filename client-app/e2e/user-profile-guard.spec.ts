...
test.describe('1. 프로필 미완성 사용자 리디렉트', () => {
  ...
  test('1-4. /profile/register 폼 완성 후 /my-page 접근 가능', async ({ page }) => {
    // devLogin: 쿠키 + Zustand localStorage 동시 설정 → email 즉시 pre-fill 보장
    // injectDevLoginCookies는 Zustand를 설정하지 않아 email 필드가 비어 검증 실패함
    await page.goto('/login', { waitUntil: 'domcontentloaded', timeout: 60_000 });
    await devLogin(page, 'USER', { skipProfileCompletion: true, email: incompleteEmail });

    // /profile/register로 이동 — spinner(authLoading=true) 사라지고 폼 필드 나올 때까지 대기
    await safeGoto(page, '/profile/register');
    await page.waitForSelector('[placeholder="Zelle 입금 시 사용하는 이름"]', { timeout: 25_000 });
    ...
    await page.getByPlaceholder('@username').fill(uniqueIg);
    ...
    // Instagram ID 중복 체크 API + 백엔드 rate limiter 쿨다운 대기
  ...
  test('2-3. 어드민 users 테이블 — buyer@test.com 상세 정보 조회', async ({ browser }) => {
    ...
    try {
      ...
      if (await userRow.isVisible({ timeout: 5_000 }).catch(() => false)) {
        ...
        // 상세 페이지에서 정보 확인
        const hasEmail = await waitForText(adminPage, 'buyer@test.com', 6_000);
        expect(hasEmail, 'user email not found in detail page').toBe(true);
      } else {
        // 행이 없으면 최소 인스타 ID라도 테이블에서 확인
        const hasUser = await waitForText(adminPage, updatedInstagramId, 10_000);
        expect(hasUser, `user with instagramId "${updatedInstagramId}" not visible`).toBe(true);
      }
    } finally {
...
test.describe('3. 프로필 미완성 → 폼 완성 → 보호 경로 접근 전체 플로우', () => {
  ...
  test('3-2. /profile/register 폼 완성 → returnTo 경로(/shop)로 복귀', async ({ page }) => {
    ...
    // /profile/register?returnTo=/shop 직접 이동 (미들웨어 리디렉트 타이밍 우회)
    // commit + 25s 대기로 authLoading spinner 사라진 뒤 폼 필드 확보
    await page
    ...
    // 이메일은 dev-login에서 자동 프리필됨
    await page.getByPlaceholder('Zelle 입금 시 사용하는 이름').fill('E2E플로우');
    await page.getByPlaceholder('@username').fill(flowIg);

    // Instagram 중복 확인 완료 대기
    await page.waitForSelector('[type="submit"]:not([disabled])', { timeout: 12_000 });
