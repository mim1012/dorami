...
function CartItemRow({
  ...
  return (
    <div
      className={`flex items-center gap-2 py-3 border-b border-white/5 last:border-0 ${isSoldOut ? 'opacity-50' : ''}`}
    >
      {/* Checkbox */}
      <input
        type="checkbox"
        ...
        className="w-4 h-4 rounded border-white/30 accent-[#FF1493] cursor-pointer flex-shrink-0 disabled:cursor-not-allowed"
      />
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className={`font-bold text-xs mt-0.5 ${isSoldOut ? 'text-white/40' : 'text-white'}`}>
          {formatPrice(item.subtotal)}
        </p>
      </div>

      {isSoldOut ? (
        /* Only delete button for sold-out items */
        <button
...
export default function LiveCartPanel({ onProceedToCheckout }: LiveCartPanelProps) {
  ...
  const selectableItems = items.filter((i) => i.product?.status !== 'SOLD_OUT');
  const allSelected =
  ...
  const handleSelectAll = useCallback(
    (checked: boolean) => {
      setSelectedIds(checked ? new Set(selectableItems.map((i) => i.id)) : new Set());
    },
    [selectableItems],
  );
