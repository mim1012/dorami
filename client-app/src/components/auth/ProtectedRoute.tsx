...
export function ProtectedRoute({ children, requiredRole }: ProtectedRouteProps) {
  ...
  const { user, isAuthenticated, isLoading, isSessionVerified, isVerifying } = useAuth();
  const currentPath =
  ...
  const hasUser = !!user;
  const authPending = isLoading || isVerifying;
    ...
    return (
      <div className="min-h-screen bg-primary-black flex items-center justify-center">
        <p className="text-primary-text">
          {hasUser ? '로그인 세션을 다시 확인하는 중입니다.' : '로그인 세션을 확인하는 중입니다.'}
        </p>
      </div>
    );
  }
