...
# 특정 서비스 로그
docker logs -f doremi-backend
docker logs -f doremi-frontend
docker logs -f doremi-rtmp
```
...
```bash
# 로그 확인
docker logs doremi-backend
docker logs doremi-frontend
...
```bash
# PostgreSQL 상태 확인
docker exec -it doremi-postgres psql -U doremi -d doremi -c '\dt'

# Redis 상태 확인
docker exec -it doremi-redis redis-cli ping
```
...
```bash
# RTMP 로그 확인
docker logs doremi-rtmp

# HLS 파일 확인
docker exec -it doremi-rtmp ls -la /tmp/hls/
```
...
```bash
# 백업
docker exec doremi-postgres pg_dump -U doremi doremi > backup_$(date +%Y%m%d).sql

# 복구
cat backup_20240101.sql | docker exec -i doremi-postgres psql -U doremi doremi
```
...
```bash
# 볼륨 위치 확인
docker volume inspect doremi_postgres_data

# tar로 백업
sudo tar -czvf postgres_backup.tar.gz /var/lib/docker/volumes/doremi_postgres_data
```
